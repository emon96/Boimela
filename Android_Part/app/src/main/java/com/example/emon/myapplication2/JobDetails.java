...
public class JobDetails extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (part2!=null)
        {
            databaseReference=FirebaseDatabase.getInstance().getReference().child(getString(R.string.jobcategory)).child(part2).child(part1);
        }
        else
        {
            databaseReference=FirebaseDatabase.getInstance().getReference().child("All Jobs").child(friend_id);
        ...
        button=findViewById(R.id.interestedbuttonId);
    ...
    public void addValue()
    {
        ...
        button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                DatabaseReference d1=myreference.push();

                d1.child("title").setValue(tit);
                d1.child("location").setValue(loc);
                d1.child("salary").setValue(sal);
                d1.child("quality").setValue(req);
                d1.child("responsibility").setValue(res);
                d1.child("other").setValue(abo);
                d1.child("category").setValue(cat);
                d1.child("contact").setValue(con);
                d1.child("deadline").setValue(dea);
                d1.child("company").setValue(com);
                d1.child("experience").setValue(ex);
            }
        });
